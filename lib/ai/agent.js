...
import { createAgentJobTool, getAgentJobStatusTool, popebotCodingTool, headlessCodingTool } from './tools.js';
...
export async function getAgentChat() {
  if (!globalThis.__popebotAgentChat) {
    ...
    const tools = [createAgentJobTool, getAgentJobStatusTool, popebotCodingTool];
 ...
 * Uses a static headlessCodingTool that reads context from runtime.config.configurable.
 */
export async function getCodeChat() {
  if (!globalThis.__popebotCodeChat) {
    const model = await createModel();
    const tools = [headlessCodingTool];

    const webSearchTool = await createWebSearchTool();
    if (webSearchTool) {
      tools.push(webSearchTool);
      console.log(`[agent] Web search enabled for code chat (provider: ${getProvider()})`);
    }
