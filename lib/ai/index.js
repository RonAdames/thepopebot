  ...
  try {
    ...
    for await (const event of stream) {
      ...
      if (msgType === 'ai') {
        ...
        if (headlessToolName === 'start_headless_coding_agent' || headlessToolName === 'plan_popebot_updates') {
          try {
...
async function summarizeAgentJob(results) {
  try {
    ...
    if (!systemPrompt) {
      console.error(`[summarizeAgentJob] Empty system prompt — AGENT_JOB_SUMMARY.md not found or empty at: ${agentJobSummaryMd}`);
    }
