 ...
 * @returns {Promise<{containerId: string, containerName: string}>}
 */
async function runInteractiveContainer({ containerName, repo, branch, codingAgent, featureBranch, workspaceId, chatContext }) {
 ...
 * @returns {Promise<{containerId: string, containerName: string}>}
 */
async function runHeadlessContainer({ containerName, repo, branch, featureBranch, workspaceId, taskPrompt, mode = 'plan', codingAgent, systemPrompt, continueSession }) {
  const agent = codingAgent || getConfig('CODING_AGENT') || 'claude-code';
  const version = process.env.THEPOPEBOT_VERSION;
  const image = `stephengpope/thepopebot:coding-agent-${agent}-${version}`;
